...
async function sendAcceptanceNotification(proposal, ip) {
  ...
  const priceStr = `$${monthlyIva.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}`;
...
<tr><td align="center">
<table width="540" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.05);">
<tr><td style="background:#059669;padding:30px;text-align:center;">
<h1 style="color:#fff;font-size:22px;margin:0;">Propuesta Aceptada</h1>
...
<strong>Precio mensual:</strong> ${priceStr} IVA incl.<br>
...
<h1 style="color:#fff;font-size:22px;margin:0;">Propuesta Confirmada</h1>
...
Confirmamos la recepcion de tu aceptacion de la propuesta de servicios Flexio para <strong>${proposal.company_name}</strong>.
</p>
...
Pablo Encina de Flexio se pondra en contacto contigo para activar el servicio.
</p>
<p style="font-size:13px;color:#6b7280;margin:16px 0 0 0;">
WhatsApp: +56 9 4961 6038
</p>
</td></tr>
<tr><td style="padding:20px 30px;border-top:1px solid #e5e7eb;text-align:center;">
<p style="font-size:11px;color:#9ca3af;margin:0;">Flexio - flexio.cl</p>
